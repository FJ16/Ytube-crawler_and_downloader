...
for element in soup.find_all('a', attrs = {"rel": "spf-prefetch"}):
        # get the value under the attribute name same with 'title'
        vedio_title = element.get('title')
        vedio_link = element.get('herf')
        print(vedio_title)
        # using {} as a temporary storage location to load content in .format(content)
        # we can also use variable in {}, such as {x} or {y} with .format(x = xxx) or .format(y = xxx)
